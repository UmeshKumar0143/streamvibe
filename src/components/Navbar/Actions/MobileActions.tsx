...
export default function MobileActions({user}:{user:any}){
    ...
    return <div>
      {!user &&  <div className="md:hidden ">
                {!isOpen?  <User onClick={()=>setisOpen(prev=>!prev)} className="cursor-pointer transition-all ease-in duration " />:<X onClick={()=>setisOpen(prev=>!prev)} className="text-muted-foreground cursor-pointer hover:font-semibold transition-all ease-in duration-100"/>}
